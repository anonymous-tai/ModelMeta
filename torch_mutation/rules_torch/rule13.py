...
class TransLayer_rule13_AvgPool2d(nn.Module):
...
class TransLayer_rule13_MaxPool2d(nn.Module):
    def __init__(self, layer_pool):
...
class TransLayer_rule13_AdaptiveAvgPool2d(nn.Module):
    def __init__(self, layer_pool):
...
if __name__ == "__main__" and False:
    ...
    np.random.seed(0)
    # 定义原始池化层
    original_pool = nn.MaxPool2d(kernel_size=2, stride=2).to(device)

    trans_pool = TransLayer_rule13_MaxPool2d(original_pool).to(device)
