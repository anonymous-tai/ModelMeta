...
def write_getmethod(model):
    layers=model.cells_and_names()
    flag = True
    f = open("./get_method.txt", "w")
    f.write("    def get_layers(self,layer_name):" + "\n")
    for layer in layers:
        layer_name = layer[0]
        if layer_name == "":
            continue
...
if __name__ == '__main__':
    ...
    # print(config)
    #
    # y = ms.Tensor(np.ones([1, 3, 224, 224]).astype(np.float32))
    # print(model(y))
    #
    #
    # # # write_getmethod(model)
    # # print(config)
    # from mindspore import Tensor
    #
    # # 创建迭代器
    ...
    #         print(nums)
    #         # print(Tensor(item['image']).shape)
    #         # outputs = model(Tensor(item['image'])).asnumpy()
    #         # # acc += (outputs.argmax(1) == item['label']).sum()
    #         # print(outputs.shape)
    #         # batch += 1
