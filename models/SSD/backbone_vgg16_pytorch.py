...
from models.SSD.ssd_utils_torch import MultiBox, class_loss
...
class SSDWithVGG16(nn.Module):
    def __init__(self):
        ...
        self.m2 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.m3 = nn.MaxPool2d(kernel_size=2, stride=2, padding=1)
        self.m4 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.m5 = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)

        # after vgg16 backbone
        self.b6_1 = nn.Conv2d(in_channels=512, out_channels=1024, kernel_size=3, padding=6, dilation=6)
        ...
        self.b8_1 = nn.Conv2d(in_channels=1024, out_channels=256, kernel_size=1, padding=1)
        self.b8_2 = nn.Conv2d(in_channels=256, out_channels=512, kernel_size=3, stride=2, padding=0)

        self.b9_1 = nn.Conv2d(in_channels=512, out_channels=128, kernel_size=1, padding=1)
        self.b9_2 = nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, stride=2, padding=0)

        self.b10_1 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=1, padding=0)
        self.b10_2 = nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, padding=0)

        self.b11_1 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=1, padding=0)
        self.b11_2 = nn.Conv2d(in_channels=128, out_channels=256, kernel_size=3, padding=0)
...
if __name__ == '__main__':
    image = torch.tensor(np.random.rand(25, 3, 300, 300), dtype=torch.float32)
    num_matched_boxes = torch.tensor([[33]], dtype=torch.int64)
    gt_label = torch.tensor(np.random.randn(1, 1917), dtype=torch.int64)  # np.load("./official/gt_label.npy")
    get_loc = torch.tensor(np.random.randn(1, 1917, 4),
                           dtype=torch.float32)  # mindspore.Tensor(np.load("./official/get_loc.npy"), mindspore.float32)
