...
import numpy as np
import mindspore
from ssd_utils import FpnTopDown, BottomUp, conv_bn_relu, ConvBNReLU, FeatureSelector, MultiBox, \
    SigmoidFocalClassificationLoss, class_loss, WeightSharedMultiBox
...
class MobileNetV1(nn.Cell):
    ...
    def __init__(self, class_num=1001, features_only=False):
        ...
        if self.features_only:
            self.net = nn.CellList(cnn)
        else:
            self.net = nn.SequentialCell(cnn)
...
class ssd_mobilenetv1fpn_ms(nn.Cell):
    def __init__(self, ):
        super(ssd_mobilenetv1fpn_ms, self).__init__()
        cnn = [
            conv_bn_relu(3, 32, 3, 2, False),  # Conv0
            conv_bn_relu(32, 32, 3, 1, True),  # Conv1_depthwise
            conv_bn_relu(32, 64, 1, 1, False),  # Conv1_pointwise
...
#                                                                                                    51150
#
#         self.multi_box = WeightSharedMultiBox(self.num_classes, self.out_channels, self.num_default, self.num_features, self.num_addition_layers,
#                                          self.num_ssd_boxes, loc_cls_shared_addition=False)
#
#
#
...
if __name__ == '__main__':
    ...
    model = ssd_mobilenetv1fpn_ms()
    print(model(image))

    # num_matched_boxes = mindspore.Tensor([[33]], mindspore.int32)
    # gt_label = mindspore.Tensor(np.random.randn(1, 8028), mindspore.int32)
    # get_loc = mindspore.Tensor(np.random.randn(1, 8028, 4),mindspore.float32)
    #
    #
    # network = SSDMobileNetV1FPN()
    #
    ...
    #
    # num_classes, out_channels, num_default, num_features, num_addition_layers, num_ssd_boxes = 81, \
    #                                                                                            256, \
    #                                                                                            6, \
    #                                                                                            5, \
    #                                                                                            4, \
    #                                                                                            51150
    #
    # multi_box = WeightSharedMultiBox(num_classes, out_channels, num_default, num_features, num_addition_layers,
    #                                  num_ssd_boxes, loc_cls_shared_addition=False)
    #
    # result = multi_box(features)
