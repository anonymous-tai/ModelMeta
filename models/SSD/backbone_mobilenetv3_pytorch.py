...
class Activation(nn.Module):
    ...
    def __init__(self, act_func):
        ...
        elif act_func in ('Hardswish', 'hard_swish'):
...
class GlobalAvgPooling(nn.Module):
    ...
    def __init__(self, keep_dims=False):
        ...
        self.mean = lambda x: torch.mean(x, dim=(2, 3), keepdim=keep_dims)
...
class MobileNetV3(nn.Module):
    def __init__(self, model_cfgs, num_classes=1000, multiplier=1., final_drop=0.,
        ...
        self.features.append(Activation('Hardswish'))
    ...
    def forward(self, x):
        ...
        if self.include_top:
            x = self.output(x)
            x = self.squeeze(x, dim=3)
            x = self.squeeze(x, dim=2)
...
if __name__ == '__main__':
    ...
    summary(model, (1, 3, 640, 640), depth=4)
