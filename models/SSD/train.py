...
from models.SSD.src.dataset import create_ssd_dataset, create_mindrecord
from models.SSD.src.lr_schedule import get_lr
from models.SSD.src.init_params import init_net_param, filter_checkpoint_parameter_by_list
from models.SSD.src.eval_callback import EvalCallBack
from models.SSD.src.eval_utils import apply_eval
from models.SSD.src.box_utils import default_boxes
from models.SSD.src.model_utils.config import config
from models.SSD.src.model_utils.moxing_adapter import moxing_wrapper
...
def set_graph_kernel_context(device_target, model):
    if device_target == "CPU" and model == "ssd300":
    ...
    if device_target == "CPU" and model == "ssd_mobilenet_v1":
        # Enable graph kernel for default model ssd300 on GPU back-end.
...
if __name__ == '__main__':
    ...
    ssd = ssd_model_build()
    import numpy as np
    x = mindspore.Tensor(np.random.randn(1, 3, 640, 640), mindspore.float32)
    features = ssd.feature_extractor(x)
    pred_loc, pred_label = ssd.multi_box(features)
