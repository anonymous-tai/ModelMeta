...
from mindspore import context, ops, Tensor, SymbolTree
...
if __name__ == '__main__':
    ...
    net = UNetMedical(n_channels=1, n_classes=2)
    # mindspore.export(net, Tensor(np.ones([1, 1, 572, 572], np.float32)), file_name="unet.onnx", file_format='ONNX')
    stree = SymbolTree.create(net)
    head_node = stree._symbol_tree.get_head_node()
    # print(head_node.get_node_type())
    # print(head_node.get_name())
    if head_node is None:
        print("head_node None, return")
    node = head_node.get_next()
    current_op_pool = set()
    while node is not None:
        current_op_pool = current_op_pool.union([node.get_node_type()])
    ...
    # train_dataset, valid_dataset = create_dataset(config.data_path, config.repeat, config.batch_size, augment=True,
    #                                               cross_val_ind=config.cross_valid_ind,
    ...
    #     return loss
    #
    # # config中的repeat值已修改来加速模型加载时间
    # print("prepare to fly")
    # start_time = time.time()
    # train_ds = train_dataset.create_dict_iterator()
    # # for data_dict in train_ds:
    ...
    #         print("time", end_time - start_time)
    #         print("================================================================")
    #         loss_ms = train_step(data["image"], data["mask"])
