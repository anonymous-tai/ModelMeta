...
if __name__ == '__main__':
    ...
    args.dataset_name="imagenet"
    args.dataset_path="/root/MSTest/data/ImageNet2012_val"
    # net = get_network(backbone_name=args.backbone, args=args)
    #dataset = get_dataset(dataset_name=args.dataset_name,
                          # do_train=True,
                          # dataset_path=args.dataset_path,
                          # args=args)
    model = get_network(backbone_name=args.backbone, args=args)

    data=mindspore.Tensor(np.random.randn(2,3,224,224),mindspore.float32)
    ...
    #         outputs = model(Tensor(item['image'])).asnumpy()
    #         print(outputs.shape)
    #         print(item['label'].shape)
    #         acc += (outputs.argmax(1) == item['label'].argmax(1)).sum()
    #         batch += 1
    # print("acc:" + str(acc / nums))
