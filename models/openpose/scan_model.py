...
from main_parallel_torch import OpenPoseNet
...
if __name__ == '__main__':
    ...
    network_ms = model_prepare(network_ms, (1, 3, 368, 368))
    ...
    result, global_layer_info = torchinfoplus.summary(model=network_torch, input_size=model_input_size,
    ...
    result, global_layer_info = mindsporeinfoplus.summary_plus(model=network_ms, input_size=model_input_size,
    ...
    util.write_layernames(network_torch, name=model_name)
    util.write_setmethod(network_torch, name=model_name)


    util.check_orderinfo_selfcorrect(network_ms)
    util.check_orderinfo_selfcorrect(network_torch)
    util.check_layers_and_shapes(network_ms)
    util.check_layers_and_shapes(network_torch)
    # compare the models constructed by two frameworks
    check_ms_torch_modelinfo(network_ms, network_torch)
