...
class DetectionEngine:
    ...
    def do_nms_for_results(self):
        """Get result boxes."""
        for img_id in tqdm(self.results):
            for clsi in self.results[img_id]:
                dets = self.results[img_id][clsi]
                # s_time = time.time()
                # print("type of dets torch:", type(dets[0][0]))

                dets = np.array(dets)
                # print("np.array time:", time.time() - s_time)
                # dets = torch.tensor(dets).to(device)
                keep_index = self._nms_np(dets, self.nms_thresh)
    ...
    def do_nms_for_results_ms(self):
        """Get result boxes."""
        for img_id in tqdm(self.results):
            for clsi in self.results[img_id]:
                dets = self.results[img_id][clsi]
                # print("type of dets ms:", type(dets[0][0]))
                # s_time = time.time()
                dets = np.array(dets)
                # print("np.array time:", time.time() - s_time)
    ...
    def _nms_np(self, predicts, threshold):
        """Calculate NMS."""
    ...
    def _nms(self, predicts, threshold):
        ...
        while order.size()[0] > 0:
            i = order[0]
            reserved_boxes.append(i.item())
            max_x1 = torch.max(x1[i], x1[order[1:]])
            max_y1 = torch.max(y1[i], y1[order[1:]])
            min_x2 = torch.min(x2[i], x2[order[1:]])
            min_y2 = torch.min(y2[i], y2[order[1:]])
            intersect_w = torch.clamp(min_x2 - max_x1 + 1, min=0)
            intersect_h = torch.clamp(min_y2 - max_y1 + 1, min=0)
            intersect_area = intersect_w * intersect_h
            ovr = intersect_area / (areas[i] + areas[order[1:]] - intersect_area)
            indexes = torch.where(ovr <= threshold)[0]
            order = order[indexes + 1]
        return reserved_boxes
    ...
    def write_result(self):
        ...
        try:
            self.file_path_ms = self.save_prefix + '/predict_ms' + t + '.json'
            f = open(self.file_path_ms, 'w')
            json.dump(self.det_boxes, f)
        except IOError as e:
            raise RuntimeError("Unable to open json file to dump. What(): {}".format(str(e)))
        else:
            f.close()
            return self.file_path_ms

    def write_result_torch(self):
        """Save result to file."""
        import json
        t = datetime.datetime.now().strftime('_%Y_%m_%d_%H_%M_%S')
        try:
            self.file_path_torch = self.save_prefix + '/predict_torch' + t + '.json'
            f = open(self.file_path_torch, 'w')
            json.dump(self.det_boxes, f)
        except IOError as e:
            raise RuntimeError("Unable to open json file to dump. What(): {}".format(str(e)))
        else:
            f.close()
            return self.file_path_torch

    def get_eval_result(self):
        """Get eval result."""
        cocoGt = COCO(self.annFile)
        cocoDt = cocoGt.loadRes(self.file_path_ms)
        ...
        cocoEval.summarize()
        sys.stdout = stdout
        return rdct.content, cocoEval.stats

    def get_eval_result_torch(self):
        """Get eval result."""
        cocoGt = COCO(self.annFile)
        cocoDt = cocoGt.loadRes(self.file_path_torch)
        cocoEval = COCOeval(cocoGt, cocoDt, 'bbox')
        cocoEval.evaluate()
        cocoEval.accumulate()
        rdct = Redirct()
