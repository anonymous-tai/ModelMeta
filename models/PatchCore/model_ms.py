...
class Patchcore(nn.Cell):
    ...
    def construct(self, x):
        ...
        x = ops.pad(x, padding=(1, 1, 1, 1), mode="reflect")
...
def wide_resnet50_2():
    return Patchcore(Bottleneck, [3, 4, 6, 3], width_per_group=64 * 2)


if __name__ == '__main__':
    net_ms = wide_resnet50_2()
    net_ms.set_train(False)
    inpu = mindspore.Tensor(np.random.randn(1, 3, 224, 224).astype(np.float32))
    out_ms = net_ms(inpu)
    print("type", type(out_ms))
    print(out_ms[0].shape, out_ms[1].shape)
