...
def np_2_tensor(datas, dtypes, device):
    result = []
    for data_, dtype_ in zip(datas, dtypes):
        result.append(torch.tensor(data_).to(device).type(dtype_))
    return result
...
def module_2_id(model):
    ...
    def make_dict(module, modules=None, name_prefix=''):
        cell_id = str(id(module))
        t_modules = modules if modules else set()
        if module in t_modules:
            return

        t_modules.add(module)
        ...
        for name, module_ in module._modules.items():
            if module_:
                ...
                for ele in make_dict(module_, t_modules, cells_name_prefix):
                    yield ele
...
def summary(
    ...
    if verbose > Verbosity.QUIET:
        # print(results)
        pass
    return results, global_layer_info,summary_list
...
def forward_pass(
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to run TorchInfoPlus. See above stack traces for more details. "
...
def construct_pre_hook(
    ...
    def pre_hook(module: nn.Module, inputs: Any) -> None:
        ...
        del inputs
        info = LayerInfo(var_name, module, curr_depth, parent_info, names)
...
def apply_hooks(
    ...
    while stack:
        ...
        global_layer_info[module_id] = LayerInfo(
            var_name, module, curr_depth, parent_info, names
        )
